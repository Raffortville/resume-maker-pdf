...
import { createResume } from '../../../Store/resumeStore';
...
const MainInfos = (props) => {
		...
		if (
			!isStringEmpty(
				personalInfo.firstName &&
					!isStringEmpty(personalInfo.lastName) &
						checkInputFormat(personalInfo.emailPro, 'email')
			)
		) {
			setDisabled(false);
	...
	const handleChange = (e, type) => {
		...
		if ((type === 'text') & (checkInputFormat(value, 'text') || value === '')) {
			...
			(type === 'phone') &
			(checkInputFormat(value, 'number') || value === '')
		) {
			setPersonnalInfo({ ...personalInfo, [name]: value });
			let phone = +value;
	...
	return (
		<Skeleton mainTitle='About you' next='/resume/form/resume-infos'>
			<div>
				<h3>First name *</h3>
				<TextField
					error={firstNamerror}
